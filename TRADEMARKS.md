# TRADEMARKS

HyperPi-Ai and Hyperπ-Ai are claimed marks of the same organization.
All associated logos, stylizations, and visual identity elements are protected and controlled by HyperPi-Ai and its founder.

## Trademark Usage Rule
No trademark license is granted by default.
You may not use the marks in names, handles, domains, branding, metadata,
SEO tags, or profile graphics without prior written permission.

## Affiliation Rule
Only HyperPi-Ai and expressly authorized affiliates may use the marks for
official representation.

## Reporting & Enforcement
Unauthorized use may be enforced through GitHub trademark reporting and other
available U.S. and Indian legal channels.

Contact: **legal@hyperpiai.com**


Official username: **HyperpiAi**
